...
## Mental model — observe facts, then author semantics

During exploration you author `_staging/walk.json`: raw observation, one entry per screen plus
edges and decision points. Assembly deterministically produces the observed `graph.json`.
After assembly, the semantic packaging pass reads a deterministic inventory and authors
`flows.json`; the static builder then derives `view.json` from both committed sources.

```
walk      →  _staging/walk.json                                        ← raw observation
...
inventory →  node scripts/flows.ts inventory {date}/graph.json        ← canonical screens/groups/edges
author    →  {date}/flows.json                                         ← semantic tree, names, steps, entries
package   →  node scripts/package.ts {date}/graph.json                 ← validates + derives view/replay
edit      →  update graph overrides or flows.json + re-run package
```

`graph.json` is the committed observation source. `flows.json` is the committed semantic
source. Screen merging, derivation classification, validation, context, replay, and the
presentation view are deterministic; grouping, flow names, parentage, and local steps are
authored according to [flow-grouping.md](references/flow-grouping.md) and
...
|---|---|
| "Capture {app}" (first time) | Full BFS → assemble graph → inventory → author flows → package. See workflow §1. |
...
| "List flows for {app}" | Run `scripts/package.ts` on the latest graph/flows pair. No device. |
| "Change/rename/reparent {X}" | Ambient edit — change `flows.json`; screen facts use graph overrides. See [editing.md](references/editing.md). |
...
{OUTPUT_DIR}/{app-slug}/                  # one dir per app — never read a sibling app's dir
  ...
  {YYYY-MM-DD}/
    ...
    flows.json              # AUTHORED semantic flows: ids + names + parents + local steps + entries
...
Each `graph.json` is a complete observed graph at a date and each `flows.json` is its reviewed
semantic package. `view.json` is a generated build artifact.
...
5. Package     a. node scripts/flows.ts inventory {date}/graph.json.
               b. Audit screen derivations; correct graph screen overrides and regenerate if needed.
               c. Author {date}/flows.json using flow-grouping.md, then naming.md.
               d. node scripts/flows.ts validate {date}/graph.json --strict.
               e. node scripts/package.ts {date}/graph.json (derive view/context/replay).
6. Wrap up     Update credentials.md + app.json manifest. Close session.
```

You do not compute identity signals, classify derivations, derive context, or write replay
scripts by hand. You do author semantic flows from the canonical inventory.
...
3. Copy the prior graph's observation overrides into the fresh walk, assemble a new graph,
   generate its inventory, and author/review a new dated `flows.json` while preserving ids for
   unchanged intents. No automatic reference re-binding is implemented.
...
Edit-shaped requests mutate `flows.json` for semantic changes or `graph.json.overrides` for
screen observation corrections, then re-run strict validation and packaging. See
[editing.md](references/editing.md). Never hand-edit the generated view.
...
- **Author flows semantically, never structurally.** Use the inventory and grouping guide; do
  not let replay or graph shape decide intent or parentage.
- **Keep the two edit surfaces separate.** Flow semantics live in `flows.json`; observed
  screen corrections live in `graph.json.overrides`.
...
| Reference | Use when |
|---|---|
| [references/schema.md](references/schema.md) | The `walk.json`, `graph.json`, `flows.json`, `view.json`, and app contracts. |
| [references/flow-grouping.md](references/flow-grouping.md) | Authoring semantic flow structure, steps, entries, and coverage. |
| [references/naming.md](references/naming.md) | Authoritative screen, derivation, flow, and stable-id naming. |
...
| [references/editing.md](references/editing.md) | Ambient edits across graph overrides and semantic flows. |
